...
public class AfterScenarioHook(BookingHelper bookingHelper, ScenarioContext scenarioContext)
{
    ...
    {
        var bookingId = scenarioContext.GetBookingId();

        if (bookingId != 0) await bookingHelper.CleanUpBookingAsync(bookingId);
    }
    ...
    public async Task AfterScenarioCleanUpAllBookings()
    {
        ...
        foreach (var id in bookingsIds) await bookingHelper.CleanUpBookingAsync(id);
    }
}